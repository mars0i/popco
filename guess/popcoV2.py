...
def showWeightSource(e):
	if e.weightsource != "":
        	createConvexHull([e.node1, e.node2, eval(e.weightsource)], gray)
...
# creates a space in which there are two buttons to show/hide weight sources:
class WeightSourceConfig(com.hp.hpl.guess.ui.DockableAdapter):
	...
	def __init__(self):
		...
		# old code that created buttons to show/hide all weight sources:

		# store what leaving an edge usually does (didn't actually use this):
		#defaultMouseLeaveEdgeFn = graphevents.mouseLeaveEdge
		#defaultMouseEnterEdgeFn = graphevents.mouseEnterEdge

		# These add the buttons
...
# Make the Information Window on the left show up by default:
InfoWindow.create()
