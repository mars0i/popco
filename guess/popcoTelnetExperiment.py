...
class telnetCommandServer(com.hp.hpl.guess.ui.DockableAdapter):
	myLabel = javax.swing.JLabel("Waiting for connection")

	def __init__(self):
		self.add(self.myLabel) # add our toolbar
		ui.dock(self)
...
class guessnetwork(java.lang.Thread):
	screeninterface = None; # keep a reference to our toolbar
        mySocket = None;
	def __init__(self):
		print "In guessnetwork.init"
		self.screeninterface = telnetCommandServer()
		self.start()	# start the thread
	def run(self):
		print "In guessnetwork.run"
		mySocket = java.net.ServerSocket(2222) # run on port 2222
		clientSocket = mySocket.accept() # accept a connection
                self.screeninterface.myLabel.setText("connected")
		print "about to new the readers"
...
def makePopcoMenu(): 
	...
	telnetItem = JMenuItem("telnet")
	popcoMenu.add(telnetItem)
	telnetItem.actionPerformed = lambda event : guessnetwork()
