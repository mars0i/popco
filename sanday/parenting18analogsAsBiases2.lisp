;;parenting18analogsAsBiases2.lisp
; like parenting18analogsAsBiases1.lisp, but fixed the bug
; that the common origin propn was left out.  Also adds
; an all-origin-propns model person.
; Based originally on parenting18untilCollect5.lisp
; See parenting18analogsAsBiases.nts for more info.
...
; utters only source (lifestyle) propns, has all origin propns
(defun make-all-origin-lifestyle-talker (name &optional (given '()))
    (make-person name 'folks given
                 `((make-struc 'target 'problem '(start (,@generic-origin-propns ,@sky-origin-propns ,@earth-origin-propns)))
                   (make-struc 'source 'problem '(start (,@parenting-propns ,@hunting-propns)))
                   ,@semantic-relations)
                 `(,@pragmatic-relations)
                 '(source)))
...
(defun make-no-origin-lifestyle-talker (name &optional (given '()))
    (make-person name 'folks given
