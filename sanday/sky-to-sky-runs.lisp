...
(load "start") ; popco
(load "sanday/parenting18untilCollect4")  ; library of functions that define persons, run popoco, etc.

(defvar *run-fn-name* 'sky-to-sky) ; change this for different script behavior
(defvar *run-fn* #'sky-to-sky) ; change this for different script behavior
...
(let ((num-extra-persons (read-from-string (first *argv*)))
  ...
  (format t "Running (funcall ~S ~S ~S ~S ~S)~%" *run-fn-name* num-extra-persons addl-ticks num-to-flip output-basename nil)
  (funcall *run-fn-name* num-extra-persons addl-ticks num-to-flip output-basename nil))
