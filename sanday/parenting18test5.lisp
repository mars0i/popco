; parenting18test5.lisp
; based on parenting18untilCollect5.lisp
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; PEOPLE WITH DIRECT PERCEPTION OF THE SPIRITUAL, SO TO SPEAK (JUST TO TEST PERCEPTION OF TARGET PROPNS) 

(defun make-earth-mystic (name &optional addl-target-propn)
  (let ((addl-target-propn-list (if addl-target-propn `(,addl-target-propn) nil)))  ; quick hack to make inserting one propn or nothing at all below simple
    (make-person name 'folks `(,@GENERIC-ORIGIN-PROPNS ,@EARTH-ORIGIN-PROPNS)
                 `((make-struc 'target 'problem '(start (,@generic-origin-propns ,@EARTH-ORIGIN-PROPNS ,@addl-target-propn-list)))
                   (make-struc 'source 'problem '(start (,@parenting-propns ,@hunting-propns)))
                   ,@semantic-relations)
                 `(,@pragmatic-relations)
...
(defun make-sky-mystic (name &optional addl-target-propn)
  (let ((addl-target-propn-list (if addl-target-propn `(,addl-target-propn) nil)))  ; quick hack to make inserting one propn or nothing at all below simple
    (make-person name 'folks `(,@GENERIC-ORIGIN-PROPNS ,@SKY-ORIGIN-PROPNS)
                 `((make-struc 'target 'problem '(start (,@generic-origin-propns ,@SKY-ORIGIN-PROPNS ,@addl-target-propn-list)))
                   (make-struc 'source 'problem '(start (,@parenting-propns ,@hunting-propns)))
                   ,@semantic-relations)
                 `(,@pragmatic-relations)
...
(defun make-omniscient-person (name &optional addl-target-propn)
  (let ((addl-target-propn-list (if addl-target-propn `(,addl-target-propn) nil)))  ; quick hack to make inserting one propn or nothing at all below simple
    (make-person name 'folks `(,@GENERIC-ORIGIN-PROPNS ,@SKY-ORIGIN-PROPNS ,@EARTH-ORIGIN-PROPNS ,@PARENTING-PROPNS ,@HUNTING-PROPNS)
                 `((make-struc 'target 'problem '(start (,@generic-origin-propns ,@EARTH-ORIGIN-PROPNS ,@SKY-ORIGIN-PROPNS ,@addl-target-propn-list)))
                   (make-struc 'source 'problem '(start (,@parenting-propns ,@hunting-propns)))
                   ,@semantic-relations)
                 `(,@pragmatic-relations)
                 '())))
...
(make-skyless-person 'skyless)
(make-earthless-person 'earthless)
(make-skyless-hunter 'skyless-hunter)
(make-earthless-parent 'earthless-parent)
(make-earth-mystic 'earth-mystic)
(make-sky-mystic 'sky-mystic)
(make-omniscient-person 'omniscient)

(init-pop)

(format t "~S~%" (get 'folks 'members))
