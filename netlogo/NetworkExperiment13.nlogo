...
;         and 
;         any? persons with [person-subnet = subnet and count my-links > average-node-degree]] [
;     set i i + 1
;     ask one-of persons with [person-subnet = subnet and count my-links > average-node-degree] [
...
;    set i i + 1
;    ask one-of persons with [person-subnet = subnet and count my-links < average-node-degree] [
;      let choice (min-one-of (other persons with [person-subnet = subnet and not link-neighbor? myself and count my-links < average-node-degree]) [distance myself])
...
;  while [i < number-to-change-degree 
;         and 
;         any? persons with [person-subnet = subnet and count my-links < average-node-degree and count my-links > 0]] [
;     set i i + 1
;     ask one-of persons with [person-subnet = subnet and count my-links < average-node-degree and count my-links > 0] [
;       ask one-of my-links [die]
;    ]
;  ]
;  
...
; Marking code from "Mouse Drag Multiple Example" in Models Lib
...
to handle-select
  ...
  ;; if no persons are selected, kill off
...
to handle-drag
  ...
  if selected? old-x old-y [
    while [mouse-down?] [
      ...
      ;; we need to move both the selected persons and the sides
      ...
      ;; and adding the results to the coordinates of the persons
      ;; and sides.
      ask selected
...
to select [x1 y1 x2 y2]   ;; x1 y1 is initial corner and x2 y2 is current corner
  ...
  set selected persons with [selected? xcor ycor]
