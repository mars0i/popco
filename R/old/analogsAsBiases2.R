...
hsums <- c(parthsums, 0, -1 * sort(parthsums)) # same but with 0, neg vals to
...
scatter2domains <- function (dom1vals, dom2vals, xlab="domain 1", ylab="domain 2", main=mainTitle, sub=subTitle, runifmax=addlScatter) {
  ...
  sd2limits <- c(sd2, -1*sd2) + mean2 # y coords ditto
  ...
  abline(v=sd1limits, h=sd2limits, col=dgray, lty=2)    # dashed gray lines at 1 standard deviation
