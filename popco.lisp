...
; (In SBCL on OSX, this seems to be the result of a pretty good random number
...
(defun popco (&key cont-prev-sess) 
  ...
  (format t "*do-converse* = ~S; *do-update-propn-nets* = ~S; *do-report-to-netlogo* = ~S~%" *do-converse* *do-update-propn-nets* *do-report-to-netlogo*)
  (if *time-runs* 
...
(defun receive-utterance (generic-propn generic-struc listener trust)
  ...
  (let* ((personal-propn (generic-to-personal-sym generic-propn))
    ...
    (when is-new-thought 
      ...
      (note-unit personal-propn *propn-init-activ*)
...
(defun update-analogy-net (person)
  ...
  ; for efficient treatment of all of the at once, and in the is-newly-added property 
  ...
  ; explicity overwritten each pop-tick from map unit activations.]
...
(defun update-proposition-net (person)
  (when *do-update-propn-nets*
    ...
    (mapc #'update-assoc-from-unit (get *the-person* 'propn-map-units)))) ; from acme-infer-personal.lisp
